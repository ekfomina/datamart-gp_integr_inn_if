    ...
    def emit(self, record):
        ...
        try:
            ...
            if not response.ok:
                ...
                rs = response.reason
                self.logger.warning("logger response status code {0}: {1}".format(sc, rs))
    ...
    def emit(self, record):
        ...
        try:
            ...
            if not response.ok:
                sc = response.status_code
                rs = response.reason
                logging.warning("logger response status code {0}: {1}".format(sc, rs))
...
def exception_hook(exc_type, exc_value, exc_traceback):
    ...
    logging.error(msg="{0}\n{1}".format(exc_value, fmt_stacktrace),
